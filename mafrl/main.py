...
for num_ue in ue_list:  # số UAV cố định
    ...
    mafrl = MAFRL(num_ues=num_ue, num_uavs=10, num_episodes=1000)
    average_energy = mafrl.run()
    ...
    mafrl_results.append(average_energy)
...
plt.plot(ue_list, mafrl_results, marker="o", label="MAFRL", linewidth=2)
...
plt.grid(True)
plt.tight_layout()
plt.savefig("baseline_vs_num_ues_mafrl.png")
